...
(function () {
  ...
  const API = '/api/grama';   // mesma origem do núcleo (cmasm.erp servido em :8010)

  /* Normaliza coords_json: aceita [[lat,lng],...] (legado) ou {"points":[{lat,lng}]} (ERP) */
  function _parseCoords(raw) {
    let parsed;
    try { parsed = typeof raw === 'string' ? JSON.parse(raw) : raw; } catch { return null; }
    const pts = Array.isArray(parsed) ? parsed : (Array.isArray(parsed && parsed.points) ? parsed.points : []);
    const out = pts.map(p => {
  ...
  function renderAreas(group, areas) {
    areas.forEach(area => {
      ...
      const coords = _parseCoords(area.coords_json);
