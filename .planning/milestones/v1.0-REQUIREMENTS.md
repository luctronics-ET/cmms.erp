# Requirements Archive: v1.0 Produção: Import + Hardening
...
- [x] **RES-01**: Sistema usa disparo `por_tempo` no cálculo de vencimento, com base em data/última execução
- [x] **RES-02**: OS grava o `departamento` (lotação) numa coluna dedicada do backend; SR pré-preenche ativo+item quando vier de um serviço
- [x] **RES-03**: Ativos não-climatização têm `local_id` religado e `refri171` recebe local atribuível
- [x] **RES-04**: Térmico real usa `locais.area_m2`/`altura_m` quando preenchidos (sem quebrar quando ausentes)
- [x] **RES-05**: Role `visualizador` é enforced nas rotas de escrita (retorna 403)

### Qualidade & Testes

- [x] **QA-01**: Suíte pytest (async, httpx + asgi-lifespan) cobre as novas rotas de manutenção importadas, sem regredir os testes existentes
- [x] **QA-02**: Migrações aditivas têm teste que roda o schema do zero e valida idempotência (`PRAGMA table_info`)

### Segurança Mínima

- [x] **SEC-01**: Login verifica senha com Argon2id (substituindo djb2), com upgrade lazy do hash no primeiro login bem-sucedido, sem quebrar o contrato `POST /api/auth/login` usado por módulos externos
- [x] **SEC-02**: Senha default hardcoded (`1234`/`170842`) é removida; ausência de hash não autentica

### Ajuda & Documentação

- [x] **DOC-01**: Usuário acessa ajuda contextual (painel/drawer) com o texto de ajuda da página/seção atual; gestor edita e salva (persistido em `ajuda_topicos`)
- [x] **DOC-02**: Repositório de documentos/formulários (modelos, POPs) com controle de versão — armazenado localmente pelo cmasm.erp (filesystem + metadados SQLite); CRUD, histórico de versões (versão/autor/data), download/visualização
- [x] **DOC-03**: Resumos e guias de normas técnicas no mesmo repositório (tipo norma/guia), organizados e replicados por categoria (categorias do PMOC)

### Limpeza Final

- [x] **CLN-01**: Após todas as features importadas serem verificadas e os testes verdes, os HTMLs legados de referência são removidos, deixando código limpo (com tag git de checkpoint antes da deleção)
