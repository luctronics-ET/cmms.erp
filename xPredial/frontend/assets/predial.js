...
function portalHref() {
  ...
  if (port === "8010") return "/cmasm-erp.html";
  if (protocol.startsWith("http")) return `${protocol}//${hostname}:8010/cmasm-erp.html`;
  return "../../xCore/cmasm-erp.html";
}
...
function setHeader(activePage, subtitle) {
  ...
  root.innerHTML = `
    ...
    <div style="margin-left:20px;display:flex;gap:10px">
       <a href="${portalHref()}" class="btn btn-s btn-sm" style="padding:4px 10px;font-size:10px">⇚ xCMASM</a>
    </div>
  `;
}
